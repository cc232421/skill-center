---
name: investor-panel
description: 51 位投资大佬评审团。给定一只股票的 dimensions.json 和 raw_data.json，让 51 位投资者各自按自己的方法论打分并输出 Pydantic Signal（signal/confidence/score/verdict/comment）。覆盖经典价值派、成长投资派、宏观对冲派、技术趋势派、中国价投派、A股游资派、量化系统派 7 大流派。当用户请求"评审团/大佬怎么看/某某会买吗/做一次大佬投票"时使用。
version: 3.3.1
author: FloatFu-true
license: MIT
metadata:
  hermes:
    ...
    related_skills: [institutional-analysis]
---

# Investor Panel · 51 贤评审团
...
- `skills/institutional-analysis/scripts/lib/investor_db.py` — 51 人元数据
- `skills/institutional-analysis/scripts/lib/seat_db.py` — 23 位游资射程规则

输出：
- `.cache/{ticker}/panel.json` — 51 个 Signal + 投票统计
...
对 23 位游资，先用 `is_in_range(nickname, ticker_features)` 判断是否在射程内：
- 在射程 → 正常评分
...
```python
{
  "panel_consensus": (bullish_count / len(investors)) * 100,
...
| F 游资 | `references/group-f-china-youzi.md` | 23 |
...
- [ ] panel.json 包含 51 个 Signal
- [ ] 每个 Signal 字段齐全
- [ ] 23 位游资里至少有 N 位返回"不适合"（除非这只票是热门题材龙头）
