...
```
skills/institutional-analysis/scripts/
├── lib/
...
- 使用方式：
  ...
  cp skills/institutional-analysis/scripts/.cache/300470.SZ/raw_data.json /tmp/legacy_raw.json
  UZI_PIPELINE=1 python3 run.py 300470.SZ --no-resume
  python3 -c "from lib.pipeline.compare import compare_files; import json; print(json.dumps(compare_files('/tmp/legacy_raw.json', 'skills/institutional-analysis/scripts/.cache/300470.SZ/raw_data.json'), ensure_ascii=False, indent=2))"
  ```
