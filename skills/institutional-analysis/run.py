...
# run.py 可能出现在两个位置：
#   1) repo 根目录（Claude Code / Codex / Cursor / dev）：SCRIPTS_DIR = ROOT/skills/institutional-analysis/scripts
#   2) skill 目录里（Hermes `hermes skills install` 后只拉子目录）：SCRIPTS_DIR = ROOT/scripts
# 探测两种 layout · 不破坏现有行为
ROOT_DIR = Path(__file__).parent.resolve()
_layout_candidates = [
    ROOT_DIR / "skills" / "institutional-analysis" / "scripts",  # repo root layout
    ROOT_DIR / "skills" / "deep-analysis" / "scripts",           # legacy layout fallback
...
def main():
    ...
    try:
        sys.path.insert(0, str(Path(__file__).parent / "skills" / "institutional-analysis" / "scripts"))
