...
当用户提出 PPT 需求时，**你（Codex）根据以下指南选择风格**，不要调用任何 Python 推荐函数。
...
```bash
python ~/.Codex/skills/ppt-generator/scripts/assemble_pptx.py \
...
图片生成依赖 `~/.Codex/skills/shared-lib/config.yaml`，包含：
